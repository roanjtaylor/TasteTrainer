...
import { openChangeset, patchOps, project, rejectOps, stageOps, type Workspace } from './changesets.ts';
import { newId } from '../util.ts';
import { DOMAINS, isCuratedDomain, singleWordTopic, slugifyTopic } from '../../../shared/types.ts';
import type { Dataset, Domain, EraGroup, Item, Subtopic } from '../../../shared/types.ts';
import { ITEM_PATCH_KEYS, type ChangeOp, type Changeset, type ChatView, type ItemPatch } from '../../../shared/chat.ts';
...
const ERAS_SCHEMA = {
  type: 'array',
  description:
    'The COMPLETE new list of named era-periods: contiguous, non-overlapping, earliest first. "start" inclusive, "end" exclusive, whole years.',
  items: {
    type: 'object',
...
export const TOOL_SPECS: ToolSpec[] = [
  ...
  },
  {
    name: 'get_dataset',
    description:
      'Read a dataset: its description, subtopics, era-periods and items. detail "compact" gives one line per item (id, name, maker, year, subtopic); "full" gives every field of every item, paged. Use compact to survey, full when you need descriptions/facts.',
    inputSchema: {
      type: 'object',
      properties: {
        dataset: DATASET_REF,
  ...
  {
    name: 'get_world_map',
    description: "Read one world's spatial map: its two axes, its regions, and which dataset sits in which region.",
    inputSchema: {
      type: 'object',
  ...
  },
  {
    ...
    description:
      "The user's own curation rulebook: how fields are mapped, the anti-popularity-bias rules, dedup, what makes an item 'defining'. Read it once before proposing new items or restructuring a field in the physical or digital world.",
  ...
  },
  {
    ...
    description:
      "Stage a change to a dataset itself: its one-word topic name, its description, its subtopic list, or its era-periods. When renaming or merging subtopics, give `subtopicRenames` so the items filed there follow; every item must still have a valid subtopic afterwards.",
    inputSchema: {
      type: 'object',
      properties: {
        dataset: DATASET_REF,
...
export function datasetHeader(ds: Dataset): string {
  const subs = ds.subtopics.length
    ...
    : '  (none)';
  const eras = ds.eraGroups?.length
    ? ds.eraGroups.map((g) => `${g.label} (${g.start}–${g.end - 1})`).join('; ')
    : '(none named)';
...
async function getWorldMapTool(ctx: ToolContext, input: any): Promise<string> {
  ...
  const map = await getWorldMap(domain);
  if (!map) return `The ${domain} world has no map yet (the user draws it from the world review screen).`;
  return JSON.stringify(map, null, 1).slice(0, RESULT_BUDGET);
}
...
function cleanEras(raw: unknown): EraGroup[] {
  return list(raw)
    .map((g: any) => ({ label: str(g?.label), start: Math.round(Number(g?.start)), end: Math.round(Number(g?.end)) }))
    .filter((g) => g.label && Number.isFinite(g.start) && Number.isFinite(g.end) && g.end > g.start)
...
async function proposeUpdateDataset(ctx: ToolContext, input: any): Promise<string> {
  ...
  if (Array.isArray(input?.eraGroups)) {
    const eraGroups = cleanEras(input.eraGroups);
    if (!eraGroups.length) throw new ToolError('eraGroups had no valid periods (each needs label, start < end).');
    if (JSON.stringify(eraGroups) !== JSON.stringify(ds.eraGroups ?? [])) {
      patch.eraGroups = eraGroups; before.eraGroups = ds.eraGroups ?? [];
    }
  }
...
async function proposeCreateDataset(ctx: ToolContext, input: any): Promise<string> {
  ...
  await stage(
    ...
    [{
      ...
      subtopics: cleanSubtopics(input?.subtopics), eraGroups: cleanEras(input?.eraGroups),
    }],
    { [id]: topic },
  );
  return `Staged a new ${domain} dataset "${topic}" with id ${id}. You can stage items into it now with propose_add_items or propose_move_items.\n${STAGED_NOTE}`;
}
